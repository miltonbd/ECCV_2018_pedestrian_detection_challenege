...
class VOCDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgpath_img_id = self._imgpath % img_id
        if not os.path.exists(imgpath_img_id):
            print("{} does not exists".format(imgpath_img_id))
            exit(404)

        img = cv2.imread(imgpath_img_id, cv2.IMREAD_COLOR)
